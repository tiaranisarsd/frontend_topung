...
const FormEditDokumentasi = () => {
  ...
  useEffect(() => {
    const getDokumentasiById = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/dokumentasi/${id}`);
        setFormData({
  ...
  const updateDokumentasi = async (e) => {
    ...
    try {
      ...
      await axios.patch(`${process.env.REACT_APP_API_URL}/dokumentasi/${id}`, formDataToSend, {
        headers: {
              ...
              {formData.gambar && !gambar && (
                <div className="mb-2">
                  {formData.gambar.endsWith('.mp4') || formData.gambar.endsWith('.webm') || formData.gambar.endsWith('.mov') ? (
                    ...
                    >
                      <source src={`${process.env.REACT_APP_API_URL}${formData.gambar}`} type="video/mp4" />
                      Your browser does not support the video tag.
                    </video>
                  ) : (
                    <img 
                      src={`${process.env.REACT_APP_API_URL}${formData.gambar}`} 
