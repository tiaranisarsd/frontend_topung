...
const FormEditJadwalKegiatan = () => {
  ...
  useEffect(() => {
    const getJadwalKegiatanById = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/jadwalKegiatan/${id}`);
        const rawTanggalWaktu = response.data.tanggal_waktu;
        const date = new Date(rawTanggalWaktu);
        const isoString = date.toISOString().slice(0, 16); 
        setTanggalWaktu(isoString);
  ...
  const updateJadwalKegiatan = async (e) => {
    e.preventDefault();
    setLoading(true);
    ...
    // Konversi tanggal ke format ISO 8601 (YYYY-MM-DDTHH:mm:ssZ)
    const date = new Date(tanggal_waktu);
    const formattedDate = date.toISOString(); // Format: YYYY-MM-DDTHH:mm:ss.sssZ

    try {
      await axios.patch(`${process.env.REACT_APP_API_URL}/jadwalKegiatan/${id}`, {
        tanggal_waktu: formattedDate,
        jenis_kegiatan: jenis_kegiatan,
        lokasi: lokasi,
        deskripsi: deskripsi
      });
      setMsg("Jadwal Kegiatan berhasil diperbarui!");
      setShowToast(true);
      setTimeout(() => {
        navigate("/jadwalKegiatan");
      }, 2000);
  ...
  return (
    <Container className="p-3">
      ...
      <h2 className="mt-5 mb-3 text-blue fw-bold">Edit Jadwal Kegiatan</h2>
      <Card style={{ maxWidth: '850px', border: 'none' }} className="bg-blue2 shadow d-flex mx-auto">
        <Card.Body>
          <Form className="text-blue" onSubmit={updateJadwalKegiatan}>
...
export default FormEditJadwalKegiatan;
