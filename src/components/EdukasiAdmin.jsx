...
const Edukasi = () => {
  ...
  const [edukasiToDelete, setEdukasiToDelete] = useState(null);
  ...
  const getEdukasi = async () => {
    ...
    const handleDeleteClick = (edukasiId) => {
    setEdukasiToDelete(edukasiId);
  ...
  const confirmDelete = async () => {
    setIsDeleting(true);
    try {
      await axios.delete(`http://localhost:5000/edukasi/${edukasiToDelete}`);
            ...
            <tbody>
              {edukasi.map((edukasi, index) => (
                <tr key={edukasi.id}>
                  ...
                  <td>{edukasi.user.nama}</td>
                  <td>
                   {users  && ( 
                     users.role === "owner" || edukasi.userId === users.id ? (
                     <><Link
                                  ...
                                  >
                                      ...
                                      variant="danger"
                                      size="sm"
                                      onClick={() => handleDeleteClick(edukasi.id)}
