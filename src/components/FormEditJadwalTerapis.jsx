...
useEffect(() => {
  const getJadwalByUserAndId = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/jadwalTerapis/users/${userId}/${id}`);
...
const updateJadwal = async (e) => {
  ...
  try {
    await axios.patch(`${process.env.REACT_APP_API_URL}/jadwalTerapis/users/${userId}/${id}`, {
      hari: hari,
      jam: jam
    });
